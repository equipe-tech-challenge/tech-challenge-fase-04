...
def detect_emotions(video_path, output_path):
    # Vetor para armazenar as emoções detectadas
    emocoes_detectadas = []
    ...
    for _ in tqdm(range(total_frames), desc="Processando vídeo"):
        ...
        for face in result:
            ...
            # Adicionar a emoção detectada ao vetor
            emocoes_detectadas.append(dominant_emotion)
    ...
    # Salvar as emoções detectadas em um arquivo .txt
    txt_path = os.path.splitext(output_path)[0] + '_emocoes.txt'
    with open(txt_path, 'w', encoding='utf-8') as f:
        for emocao in emocoes_detectadas:
            f.write(emocao + '\n')
    print(f"Emoções salvas em: {txt_path}")
...
output_video_path = os.path.join(script_dir, 'output_detect_expression_video.mp4')  # Nome do vídeo de saída
